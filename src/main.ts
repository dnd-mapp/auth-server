...
async function bootstrap() {
    const { AUTH_SERVER_SSL_CERT_PATH, AUTH_SERVER_SSL_KEY_PATH } = process.env;
    const { adapter, ssl } = await configureFastifyAdapter(AUTH_SERVER_SSL_CERT_PATH, AUTH_SERVER_SSL_KEY_PATH);
